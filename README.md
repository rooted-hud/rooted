...
```
This does a graph traversal from the webpage (to a depth of 2) and converts all `.html` and `.pdf` files to `.md`. Use `-o ./path/` to specify an output folder.
...
```
Make sure to change `const API_URL = ` in `script.js` to the appropriate ip (or `localhost:8000/chat`)
